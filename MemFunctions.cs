...
// Type: Skill_Sender.MemFunctions
...
namespace Skill_Sender
{
  public class MemFunctions
  {
    ...
    private static extern bool VirtualFreeEx(
      ...
      IntPtr lpAddress,
      int dwSize,
      MemFunctions.FreeType dwFreeType);
    ...
    public static int AllocateMemory(IntPtr processHandle, int memorySize) => (int) MemFunctions.VirtualAllocEx(processHandle, (IntPtr) 0, (uint) memorySize, 4096U, 64U);

    public static IntPtr CreateRemoteThread(IntPtr processHandle, int address) => MemFunctions.CreateRemoteThread(processHandle, (IntPtr) 0, 0U, (IntPtr) address, (IntPtr) 0, 0U, (IntPtr) 0);

    public static void WaitForSingleObject(IntPtr threadHandle)
    {
      if (MemFunctions.WaitForSingleObject(threadHandle, uint.MaxValue) == 0U)
        return;
      Debug.WriteLine("Failed waiting for single object");
    }

    public static void FreeMemory(IntPtr processHandle, int address) => MemFunctions.VirtualFreeEx(processHandle, (IntPtr) address, 0, MemFunctions.FreeType.Release);

    public static IntPtr OpenProcess(int pId) => MemFunctions.OpenProcess(2035711U, 0, (uint) pId);

    public static void CloseProcess(IntPtr handle) => MemFunctions.CloseHandle(handle);
    ...
    {
      int nSize = Marshal.SizeOf(anyType);
      byte[] numArray = new byte[nSize];
      uint lpNumberOfBytesRead = 0;
      MemFunctions.ReadProcessMemory(processHandle, (IntPtr) address, numArray, (uint) nSize, ref lpNumberOfBytesRead);
    ...
    private static string ByteArrayToString(byte[] bytes) => MemFunctions.ByteArrayToString(bytes, MemFunctions.EncodingType.Unicode);
